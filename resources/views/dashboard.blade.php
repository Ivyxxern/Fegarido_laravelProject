...
<body class="bg-gray-100 text-gray-900 dark:text-gray-100 dark:bg-gray-900 min-h-screen flex flex-col">

<div class="container mx-auto p-6">

    {{-- Session Success Message --}}
    @if(session('success'))
        <div class="rounded-lg bg-green-100 p-4 text-green-700 dark:bg-green-900/30 dark:text-green-300 mb-6">
            {{ session('success') }}
        </div>
    @endif
                ...
                <form action="{{ route('students.store') }}" method="POST" class="grid gap-4 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="Enter student name" required
                               class="w-full rounded-lg border border-neutral-300 bg-white px-4 py-2 text-sm focus:border-blue-500 
                               focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:border-neutral-600 dark:bg-neutral-800 dark:text-neutral-100">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="tel" name="phone" value="{{ old('phone') }}" placeholder="Enter phone number" required
                               class="w-full rounded-lg border border-neutral-300 bg-white px-4 py-2 text-sm focus:border-blue-500
                               focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:border-neutral-600 dark:bg-neutral-800 dark:text-neutral-100">
                        @error('phone')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            ...
                            @forelse($students as $student)
                                <tr class="transition-colors hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                                    ...
                                    <td class="px-4 py-3 text-sm">
                                        <button class="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300">Edit</button>
                                        <span class="mx-1 text-neutral-400">|</span>
                                        <form method="POST" action="{{ route('students.destroy', $student->id) }}" class="inline" 
                                              onsubmit="return confirm('Are you sure you want to delete this student?');">
                                            @csrf
                                            @method('DELETE')
                                            <button class="text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300">Delete</button>
                                        </form>
                                    </td>
                                </tr>
                            @empty
